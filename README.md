...
Devo essere nella home directory del repository e devo avere attivato l'ambiente virtuale `cmdstan_env`. Questo mi consente di usare si R che Python nello stesso file qmd.

La compilazione produce

```bash
Watching files for changes
Browse at http://localhost:5454/
```

il che consente di visionare il sito locale.

Fare dei cambiamenti nei file `.qmd`, salvare, e fare refresh sul file html consente di vedere le modifiche senza dovere compilare nuovamente tutto.
